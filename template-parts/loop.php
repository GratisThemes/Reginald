...
if( have_posts() ):
	while( have_posts() ): the_post(); ?>
		<article <?php post_class(); ?>>
			...
			<?php if( get_theme_mod( 'display_content', false ) ):
				the_content( sprintf(
					__( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'reginald' ),
					get_the_title()
				) );
			else:
				the_excerpt();
	...
	the_posts_pagination( array(
		'prev_text'          => __( 'Previous page', 'reginald' ),
		'next_text'          => __( 'Next page', 'reginald' ),
		'before_page_number' => '<span class="meta-nav screen-reader-text">' . __( 'Page', 'reginald' ) . ' </span>',
	) );
